...
user='backup_user' #An unprivileged user account that has login's disabled
home_dir=/home/$user
web_user='www-data' #Apache2
s3_bucket='' #Your s3 compatiable bucket name here
...
Email_Address=''# Insert Your E-mail address here, used to send the password for encrypted backups and panic files
...
mysqldump --single-transaction -h localhost -u $DB_User -p$DB_pass nextcloud > $backup_dir/nextcloud-sqlbkp_$timestamp.bak
...
if [ $day_of_month -eq 1 ]; then
        ...
        sudo s3cmd sync $nextcloud_dir s3://$s3_bucket/Nextcloud_Backups/ --delete-removed --human-readable-sizes --ssl --config=$home_dir/.s3cfg
        sudo s3cmd sync $data_dir s3://$s3_bucket/Data_Backups/ --delete-removed --human-readable-sizes --ssl --config=$home_dir/.s3cfg
else

        sudo s3cmd sync $nextcloud_dir s3://$s3_bucket/Nextcloud_Backups/ --human-readable-sizes --ssl --config=$home_dir/.s3cfg
